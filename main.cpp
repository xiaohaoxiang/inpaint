...
#define _OutPut(MT)                                                                                                    \
    ...
    {                                                                                                                  \
        ...
        outstr << curPicName << " time:" << t << " MSE:" << MSE(picRepaired, picRaw)                                   \
               << " PSNR:" << PSNR(picRepaired, picRaw) << " MSSIM:" << MSSIM(picRepaired, picRaw) << std::endl;       \
...
#define _TimingRun1(FuncName, MT, FileName)                                                                            \
    ...
    {                                                                                                                  \
        ...
        t = clock() - t;                                                                                               \
        const std::string curPicName =                                                                                 \
            std::string("repaired-") + #MT + "-" + std::to_string(i + 1) + "-" + #FuncName + "-" + FileName;           \
        _OutPut(MT);                                                                                                   \
    } while (false)
...
#define _TimingRun2(FuncName, F, MT, FileName)                                                                         \
    ...
    {                                                                                                                  \
        ...
        t = clock() - t;                                                                                               \
        const std::string curPicName = std::string("repaired-") + #MT + "-" + std::to_string(i + 1) + "-" +            \
                                       #FuncName + "-" + #F + "-" + FileName;                                          \
        _OutPut(MT);                                                                                                   \
    } while (false)
...
#define TimingRun(MT, FileName)                                                                                        \
    ...
    {                                                                                                                  \
        ...
        cv::imwrite(outputDir + #MT + "/damaged-" + #MT + "-" + std::to_string(i + 1) + "-" + FileName + ".png",       \
                    picDamaged);                                                                                       \
...
int main(int argc, char *const argv[])
{
    ...
    const int Rows = (argc >= 4 ? std::max(std::atoi(argv[2]), 300) : 600),
              Cols = (argc >= 4 ? std::max(std::atoi(argv[3]), 400) : 800), Pixs = Rows * Cols;
